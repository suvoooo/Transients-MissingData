...
# Define model (these are the default parameters from https://github.com/locuslab/TCN/)
input_channels = 6
...
model = models.TCN(input_channels, n_classes, channel_sizes, kernel_size, dropout, weights_tensor, labeling_order, device, DTYPE).to(device)
...
for epoch in pbar:
	...
	for b in pbar_batch:
		loss_training = model.train_step(X = X_training[b*bs:(b+1)*bs], Y = Y_training[b*bs:(b+1)*bs], optimizer = optimizer)
	...
	for i in range( n_b_val ):
		# If using NLL Loss, it is needed to get the indices from the labels. Comment the next line if it is not the case:
		labels = torch.argmax( real_label_val[i*bs_val:(i+1)*bs_val,:], dim = 1)
		loss_val.append( float( model.criterion( model(X_val[i*bs_val:(i+1)*bs_val]), labels ).detach().cpu().numpy() ) )
