...
model = models.TransformerClassifier(input_dim = input_dim, n_classes = n_classes, d_model = d_model, nhead = nhead, num_layers = num_layers, weights_tensor = weights_tensor, DTYPE = DTYPE, device = device, with_errors = True, multiply_errors = False, labeling_order = labeling_order).to(device)
...
for epoch in pbar:
	...
	for b in pbar_batch:
		loss_training = model.train_step( X = torch.cat( (X_training[b*bs:(b+1)*bs,1,:,:], X_training[b*bs:(b+1)*bs,2,:,:]), dim = 2 ), Y = Y_training[b*bs:(b+1)*bs], optimizer = optimizer )
	...
	for i in range( n_b_val ):
		loss_val.append( float( model.criterion( model( torch.cat( (X_val[i*bs_val:(i+1)*bs_val,1,:,:], X_val[i*bs_val:(i+1)*bs_val,2,:,:]), dim = 2 ) ), real_label_val[i*bs_val:(i+1)*bs_val,:] ).detach().cpu().numpy() ) )
