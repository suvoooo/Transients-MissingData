...
# Define model
input_dim = 6 + 1 # We are also considering an extra channel that corresponds to the redshift
n_classes = len(labeling_order)
d_model = 32
nhead = 4
num_layers = 4

model = models.TransformerClassifier_with_errors_and_photoz(input_dim = input_dim, n_classes = n_classes, d_model = d_model, nhead = nhead, num_layers = num_layers, weights_tensor = weights_tensor, DTYPE = DTYPE, device = device, labeling_order = labeling_order).to(device)
...
for epoch in pbar:
	...
	for b in pbar_batch:
		loss_training = model.train_step( X = X_training[b*bs:(b+1)*bs,:,:,:], Y = Y_training[b*bs:(b+1)*bs], photoz = photoz_training[b*bs:(b+1)*bs], optimizer = optimizer )
	...
	## Save metrics
	accuracy_training, cm_percent_training = model.compute_metrics( X = X_training, Y = Y_training, photoz = photoz_training, bs = bs, nb = n_b )
	...
	# Validation
	## Save metrics
	accuracy_val, cm_percent_val = model.compute_metrics( X = X_val, Y = Y_val, photoz = photoz_val, bs = bs_val, nb = n_b_val )
	...
	for i in range( n_b_val ):
		loss_val.append( float( model.criterion( model( X_val[i*bs_val:(i+1)*bs_val,:,:,:], photoz_val[i*bs_val:(i+1)*bs_val] ), real_label_val[i*bs_val:(i+1)*bs_val,:] ).detach().cpu().numpy() ) )
