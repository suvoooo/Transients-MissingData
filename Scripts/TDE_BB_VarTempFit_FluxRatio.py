'''
stes: 
1. fit the lcs with analytical model gaussian rise + exp-decay
2. obtain the flux ratios 
3. using the flux ratios, obtain Black-Body temperature (assume it doesn't change as we 
collect data over time). 
#### changes from previous TDE_BB_TempFit_FluxRatio.py # 28/7/2025
Instead of fix BB Temp, we consider Temp evolution
'''
...
### added 24/7/2025
def temperature_loss_for_ratios(T, target_ratios, weight_by_keys=None):
    """
    Loss between theoretical BB Fl ratios (check the func. above) at temperature T and target for a given time.
    target_ratios: dict like {'g/r': val, 'r/i': val, ...} for 1 epoch 
    # check the return of comp_theoretical_flux_rat function
    weight_by_keys: optional dict of weights per ratio-key (e.g. inverse-variance) # maybe needed later on 
    """
    theo = compute_theoretical_flux_ratios(T)
    loss = 0.0 # init
    for k, val in target_ratios.items():
        if np.isfinite(val):
            w = 1.0 if (weight_by_keys is None or k not in weight_by_keys) else weight_by_keys[k]
            loss += w * (theo[k] - val)**2
    return loss



def compute_T_from_flux_ratios_at_time(target_ratios, T_init=12000.0, bounds=(3000.0, 50000.0),
                                       weight_by_keys=None):
    """
    Solve for T that best matches the given ratios at one epoch.
    Returns np.nan if minimization fails.
    """
    try:
        res = minimize(
            lambda x: temperature_loss_for_ratios(x[0], target_ratios, weight_by_keys),
            x0=np.array([T_init], dtype=float),
            bounds=[bounds],
            method="L-BFGS-B",
        )
        if res.success:
...
def _ratios_from_band_fluxes(band_fluxes):
    """
    band_fluxes: dict of scalar fluxes at a single epoch, keys must include any used below.
    Returns ratio dict like what we had before.
    !! should be checked later--- to add more ratio combs !!
    """
    F = band_fluxes # dict
    out = {}
    # safety valve against negative or missing
    def safe_ratio(num, den):
        return np.nan if (num is None or den is None or den <= 0 or num <= 0) else (num / den)

    out['g/r'] = safe_ratio(F.get('g'), F.get('r'))
    out['r/i'] = safe_ratio(F.get('r'), F.get('i'))
    out['i/z'] = safe_ratio(F.get('i'), F.get('z'))
    out['z/y'] = safe_ratio(F.get('z'), F.get('y'))
    out['u/g'] = safe_ratio(F.get('u'), F.get('g'))
    return out



def compute_T_series_from_model(sampled_times, model_fluxes_by_band, T_init=12000.0):
    """
    here once fitted TDEScore model, use it to compute T(t) over sampled_times.
    model_fluxes_by_band: dict {'u': array, 'g': array, 'r': array, 'i': array, 'z': array, 'y': array}
    Returns: times (np.array), T_series (np.array)
    """
    T_list = []
    for idx in range(len(sampled_times)):
        band_fluxes_now = {
            'u': model_fluxes_by_band.get('u', None)[idx] if model_fluxes_by_band.get('u') is not None else None,
            'g': model_fluxes_by_band.get('g', None)[idx] if model_fluxes_by_band.get('g') is not None else None,
            'r': model_fluxes_by_band.get('r', None)[idx] if model_fluxes_by_band.get('r') is not None else None,
            'i': model_fluxes_by_band.get('i', None)[idx] if model_fluxes_by_band.get('i') is not None else None,
            'z': model_fluxes_by_band.get('z', None)[idx] if model_fluxes_by_band.get('z') is not None else None,
            'y': model_fluxes_by_band.get('y', None)[idx] if model_fluxes_by_band.get('y') is not None else None,
        }
        ratios_now = _ratios_from_band_fluxes(band_fluxes_now)
        if all((not np.isfinite(v)) for v in ratios_now.values()):
            T_list.append(np.nan)
            continue
        T_now = compute_T_from_flux_ratios_at_time(ratios_now, T_init=T_init)
        T_list.append(T_now)
        # warm start next epoch with previous converged T if valid
        if np.isfinite(T_now):
            T_init = T_now
    return np.array(sampled_times), np.array(T_list)


#############################################
# Can we get T(t) directly from observations?
# optional
#############################################

def compute_T_series_from_observations(obs_df, time_grid, max_join_dt=0.5, T_init=12000.0):
    """
    For each t in time_grid, gather nearest flux per band within ±max_join_dt days,
    form ratios, solve T. Useful if we prefer not to use the fitted model fluxes.
    obs_df must have columns: ['scaled_mjd','passband','flux','flux_err'] and passband in {0..5}.
    """
    band_map = {0:'u', 1:'g', 2:'r', 3:'i', 4:'z', 5:'y'}
    # Split by band for quick nearest-neighbor lookups
    per_band = {b: obs_df[obs_df['passband'] == p].sort_values('scaled_mjd')
                for p, b in band_map.items()}
    times_out, Ts = [], []
    for t in time_grid:
        band_fluxes_now = {}
        for b, dfb in per_band.items():
            if dfb.empty:
                band_fluxes_now[b] = None
                continue
            idx = np.argmin(np.abs(dfb['scaled_mjd'].to_numpy() - t))
            dt = np.abs(dfb['scaled_mjd'].to_numpy()[idx] - t)
            if dt <= max_join_dt:
                fl = float(dfb['flux'].to_numpy()[idx])
                band_fluxes_now[b] = fl if np.isfinite(fl) and fl > 0 else None
            else:
                band_fluxes_now[b] = None
        ratios_now = _ratios_from_band_fluxes(band_fluxes_now)
        if all((not np.isfinite(v)) for v in ratios_now.values()):
            ...
            if np.isfinite(T_now):
                T_init = T_now
        times_out.append(t)
    return np.array(times_out), np.array(Ts)
...
for obj_id in meta_training_tde['object_id']:
    ...
    common_t = sampled_dates_arr_g # should be checked later on 
    # possibly go with min mjd?
    ...
    def interp_to_common(sample_t, sample_flux, ref_t):
        '''
        thinking of this is like comparing flux (ratios) at the same point
        Without interp, g_fit_flux[idx] / r_fit_flux[idx], we may have diff MJDs 
        (e.g., g at MJD 60010.2 vs r at MJD 60009.7), 
        may introduces biases in comparing ratios and therefore temperature.
        '''
        if len(sample_t)< 2:
            return np.full_like(ref_t, np.nan, dtype=float)
        f = interp1d(sample_t, sample_flux, kind='linear', 
                     bounds_error=False, fill_value=np.nan)
        return f(ref_t)    
    
    model_fluxes_by_band = {'g': g_fit_flux, 
                            'r': interp_to_common(sampled_dates_arr_r, r_fit_flux, common_t), 
                            'i': interp_to_common(sampled_dates_arr_i, i_fit_flux, common_t), 
                            'u': interp_to_common(sampled_dates_arr_u, u_fit_flux, common_t), 
                            'z': interp_to_common(sampled_dates_arr_z, z_fit_flux, common_t), 
                            'y': interp_to_common(sampled_dates_arr_y, y_fit_flux, common_t)}
    
    T_times, T_series_model = compute_T_series_from_model(common_t, model_fluxes_by_band, 
                                                          T_init=T_initial) 
    
    # calc med and mean of T(t)
    T_med = np.nanmedian(T_series_model) if np.any(np.isfinite(T_series_model)) else np.nan
    T_mean = np.nanmean(T_series_model) if np.any(np.isfinite(T_series_model)) else np.nan
    ...
    fit_results.append({"object_id": obj_id, "t_peak": t_peak_fit, "sigma": sigma_fit, 
                        ...
                        "T_fit_const": T_fit, # temp constant
                        "T_times": list(T_times), 
                        "T_series": list(T_series_model),
                        "T_median": T_med, 
                        "T_mean": T_mean,
                        "redshift": obj_specz, 
...
fit_results_df.to_csv('./TDE_TDEScore_BB_FlRatio_VarTemp.csv', index=False)
